	...
	QLogicaeCppCoreTest
{
	...
	TEST_F(
		BcryptHashCryptographyManagerTest,
		Should_HandleErrorOutputs_When_ExceptionProvided
	)
	{
		try
		{
	...
	TEST_F(
		BcryptHashCryptographyManagerTest,
		Should_HandleConcurrentErrorHandling_When_MultipleThreadsUsed
	)
	{
		std::atomic<bool>
			all_successful =
	...
	TEST_F(
		BcryptHashCryptographyManagerTest,
		Should_HandleAsyncErrorHandling_When_UsingPromiseAndFuture
	)
	{
		std::promise<bool>
			result_promise;
